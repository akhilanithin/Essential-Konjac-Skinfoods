...
interface MediaThreeProps {
    product: Product;
}

const MediaThree: React.FC<MediaThreeProps> = ({ product }) => {
    ...
    return (
        <>
            <div className="product-gallery row">
                {lgImages.slice(0, 5).map((image, idx) =>
                    idx === 0 ? (
                        ...
                        <div className={`thumb-pictures ${idx % 4 < 2 ? 'col-sm-4' : 'col-sm-8'}`} key={`image-${idx}`}>
