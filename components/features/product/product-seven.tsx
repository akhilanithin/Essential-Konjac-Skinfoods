...
const ProductSeven: React.FC<ProductSevenProps> = (props) => {
    ...
    const addToCartHandler = (e: React.MouseEvent<HTMLAnchorElement>) => {
        e.preventDefault();
        if (product.stock > 0) {
            const qty = parseInt(e.currentTarget.closest('.product-with-qty').querySelector('.product-quantity .quantity').value || '0', 10);
...
export default connector(ProductSeven);
