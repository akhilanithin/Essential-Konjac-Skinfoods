...
const ShopHorizontalFilter: React.FC = () => {
    ...
    const containsAttrInUrl = (type: string, value: string) => {
        ...
        return currentQueries.includes(value) || (type === 'max_price' && value === '') || (type === 'min_price' && value === '');
    };
    ...
    return (
        <main className="main">
            <Helmet>
                <title>Shop Horizontal Filter</title>
            </Helmet>

            <h1 className="d-none">Shop Horizontal Filter</h1>
            ...
            <div className="page-content mb-lg-10 mb-0 pb-lg-3 pb-0">
                <div className="container">
                    ...
                    <div className="select-items">
                            ...
                            containsAttrInUrl('sizes', item.slug) ? (
                                <ALink
                                    ...
                                    href={{
                                        ...
                                        query: { ...query, page: 1, sizes: getUrlForAttrs('sizes', item.slug) },
                                    }}
                                    key={`${item.slug}-${index}`}
                                    onClick={selectFilterHandler}
                                    scroll={false}
                                >
                                    {item.name}<i className="d-icon-times"></i>
                                </ALink>
                            ) : null
                        )}
                        {filterData.colors.map((item, index) =>
                            containsAttrInUrl('colors', item.slug) ? (
                                <ALink
                                    ...
                                    href={{
                                        pathname: router.pathname,
                                        query: { ...query, page: 1, colors: getUrlForAttrs('colors', item.slug) },
                                    }}
                                    key={`${item.slug}-${index}`}
                                    onClick={selectFilterHandler}
                                    scroll={false}
                        ...
                        {prices.map((price, index) =>
                            containsAttrInUrl('min_price', price.min) && containsAttrInUrl('max_price', price.max) ? (
                                <ALink
                                    ...
                                    href={{
                                        pathname: router.pathname,
                                        query: { ...query, page: 1, min_price: getUrlForAttrs('min_price', price.min), max_price: getUrlForAttrs('max_price', price.max) },
                                    }}
                                    key={`${price.min}-${price.max}-${index}`}
                                    onClick={selectFilterHandler}
                                    scroll={false}
                                >
                        ...
                        <ALink
                            href={{ pathname: router.pathname, query: { type: query.type || null } }}
